...
**When using commands that use images, i.e. `/media-progress` or `/recent-sessions`, 
...
| `DEFAULT_PROVIDER`       | Set the default search provider for wishlist book searches (`audible`, `google`, `openlibrary`, `itunes`, `fantlab`, etc.). Default is `audible`.        | *String*  | **NO**    |
...
- **Dashboard & Monitoring**: View live bot & Audiobookshelf connection status, server latency, active playback sessions, and registered tasks.
- **Live Configuration**: Configure bot behavior, Audiobookshelf credentials, and Discord settings live without restarting.
- **Connection Diagnostics**: Test connections to your Audiobookshelf server or custom endpoints directly from the UI.
- **Interactive Listening Recap**: Generate personalized listening recap cards for custom date ranges, preview stats and cover collages, and send recap embed cards directly to Discord users or channels!
...
## Installation
**Current installation method is by docker container; however, you can also run main.py or launcher.py within a project folder.**
...
Bookshelf Traveller includes an automated unit test suite in the `tests/` directory to verify database operations, API integration, recap logic, and settings.
...
- **`tests/test_webui.py`**: Tests FastAPI Web UI dashboard endpoints (`/`, `/api/status`, `/api/config`), configuration updates, and settings storage.
- **`tests/test_listening_recap.py`**: Tests listening recap logic, session timestamp parsing, listening streaks, custom date range filtering, aggregated metrics calculation, and recap card generation.
...
| `/add-book`                 | Add a book to your wishlist, this command is open server-wide.                                             | `title`, `provider`, `force`                       | Most successful searches will appear on audible, but others are available (`audible`, `google`, `openlibrary`, `itunes`, `fantlab`). Use a series name when searches return too many options.                                                 | **Autocomplete**                                                                                                             |
| `/add-user`                 | Will create an Audiobookshelf user, requires username, password.                                           | `name`, `password`, `user_type`, optional: `email` | Only with ABS admin token. Otherwise disabled. *MULTI_USER must be False.                                                                                                                                                                     |                                                                                                                              |
...
| `/listening-stats`          | Pulls the current ABS user's total listening time.                                                         |                                                    |                                                                                                                                                                                                                                               |                                                                                                                              |
| `/login`                    | Login using ABS username and password.                                                                     | `username`, `password`                             | Stores your user credentials securely per Discord user.                                                                                                                                                                                       |                                                                                                                              |
...
| `/refresh`                  | Refresh / re-send current playback card.                                                                   |                                                    |                                                                                                                                                                                                                                               |                                                                                                                              |
| `/remove-book`              | Manually remove a book or mark it as downloaded in your wishlist.                                          |                                                    |                                                                                                                                                                                                                                               |                                                                                                                              |
...
| `/stop`                     | Disconnect bot from channel and stop audio.                                                                |                                                    |                                                                                                                                                                                                                                               |                                                                                                                              |
...
| `/toggle-podcast-autoplay`  | Toggle automatic podcast episode progression on or off.                                                    |                                                    | Automatically queues and plays subsequent podcast episodes upon completion.                                                                                                                                                                   |                                                                                                                              |
| `/toggle-series-autoplay`   | Toggle automatic series progression on or off.                                                             |                                                    | Automatically queues and plays the next book in a series upon completion.                                                                                                                                                                     |                                                                                                                              |
| `/user`                     | Will display the currently logged in user.                                                                 |                                                    |                                                                                                                                                                                                                                               |                                                                                                                              |
| `/user-search`              | Search for a specific user by name.                                                                        | `name`                                             | Only with ABS admin token. Otherwise disabled. *MULTI_USER must be False.                                                                                                                                                                     | **Autocomplete Enabled**                                                                                                     |
| `/view-all-wishlists`       | View all active or inactive wishlists across all users. Admin/owner command only.                          |                                                    |                                                                                                                                                                                                                                               |                                                                                                                              |
...
#### /listening-recap
Generate personalized listening summaries across any timeframe (e.g. 7 days, 30 days, 90 days, 365 days, or custom start & end dates). View total listening time, active streaks, peak listening days, favorite authors, top genres, and your most played audiobook complete with cover artwork!

#### Autoplay & Series Progression
Never lose your momentum. Toggle series autoplay (`/toggle-series-autoplay`) and podcast autoplay (`/toggle-podcast-autoplay`) so the bot automatically transitions to the next book in the series or the next episode when you finish listening.

#### Multi-Channel Notifications (/setup-tasks)
